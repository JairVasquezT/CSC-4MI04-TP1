...
cv2.imwrite('../imgs/metodo_directo.png', img2.astype(np.uint8))
...
img3_clip = np.clip(img3, 0, 255)
cv2.imwrite('../imgs/metodo_filter2d.png',img3_clip.astype(np.uint8))

diff = np.abs(img2[1:-1, 1:-1] - img3[1:-1, 1:-1])
print("Diff (interior) max =", diff.max(), "mean =", diff.mean(), "std =", diff.std())

print("img3 RAW min/max =", img3.min(), img3.max())
inside = img3[1:-1,1:-1]
print("Interior <0:", np.sum(inside < 0), "Interior >255:", np.sum(inside > 255))
...
plt.subplot(133)
plt.imshow(img3_clip,cmap = 'gray',vmin = 0.0,vmax = 255.0)
#Convention Matplotlib : par défaut, normalise l'histogramme !
plt.title('Convolution - filter2D_clip') 
